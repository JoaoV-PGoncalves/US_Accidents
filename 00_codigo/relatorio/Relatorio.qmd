...
condicoes_climaticas <- acidentes_US_sample
duracao_acidente <- acidentes_US_sample
poi <- acidentes_US_sample
...
Com apenas as colunas de interesse, a tabela `acidentes_US_date_sample` foi criada. A partir dela, criamos novas variáveis para enriquecer a análise dos dados.
...
# Exibe as primeiras observacoes da tabela 'acidentes_US_date_sample'
acidentes_US_date_sample %>% head(10)
```

Esse data.table `acidentes_US_date_sample` foi salvo em um arquivo `. RData` para ser utilizado posteriormente. Ele será a tabela base para todas as análises e gráficos realizados para o grupo 'Tempo'.
...
```{r}
#| label: Agrupando valores parecidos em 'Weather Condition'


condicoes_climaticas <- condicoes_climaticas %>%
  mutate(Weather_Cluster = case_when(
    grepl("Rain|Drizzle|Shower|Showers|Light Rain|Heavy Rain|Rain Showers|Light Rain Showers|Rain and Sleet|Rain Shower|Rain / Windy|Rain Showers", `Weather Condition`, ignore.case = TRUE) ~ "Rain",
    grepl("Snow|Blowing Snow|Light Snow|Heavy Snow|Snow Grains|Snow Showers|Snow and Thunder|Snow / Windy|Blowing Snow / Windy|Light Snow Shower|Heavy Snow / Windy|Heavy Snow with Thunder|Snow and Sleet", `Weather Condition`, ignore.case = TRUE) ~ "Snow",
    grepl("Thunder|Tornado|Thunderstorm|Light Thunderstorm|Heavy Thunderstorms and Rain|Thunder / Windy|Thunder / Wintry Mix|Thunderstorms and Snow|Light Thunderstorms and Snow|Heavy Thunderstorms with Small Hail", `Weather Condition`, ignore.case = TRUE) ~ "Thunderstorm",
    grepl("Wind|Windy|Blowing Dust|Blowing Snow|Duststorm|Dust Whirls|Blowing Dust / Windy|Widespread Dust|Sand / Windy|Windy", `Weather Condition`, ignore.case = TRUE) ~ "Wind",
    grepl("Fog|Haze|Shallow Fog|Patches of Fog|Mist|Light Fog|Fog / Windy|Partial Fog", `Weather Condition`, ignore.case = TRUE) ~ "Fog",
    grepl("Clear|Fair|Mostly Cloudy|Partly Cloudy|Overcast|Cloudy", `Weather Condition`, ignore.case = TRUE) ~ "Clear",
    grepl("Smoke|Volcanic Ash|Haze", `Weather Condition`, ignore.case = TRUE) ~ "Hazardous", 
    grepl("Sleet|Ice Pellets|Freezing Rain|Light Freezing Rain|Freezing Drizzle|Light Freezing Drizzle|Heavy Freezing Rain", `Weather Condition`, ignore.case = TRUE) ~ "Ice",
    grepl("Squalls|Hail|Small Hail|Thunder / Wintry Mix|Hail", `Weather Condition`, ignore.case = TRUE) ~ "Hail",
    grepl("Mist|Blowing Snow Nearby|Sand|Blowing Snow", `Weather Condition`, ignore.case = TRUE) ~ "Dust & Mist",
    grepl("Volcanic Ash|Blowing Sand|Sand", `Weather Condition`, ignore.case = TRUE) ~ "Dust Storm",
    grepl("Fair / Windy|Fair", `Weather Condition`, ignore.case = TRUE) ~ "Fair / Windy",
    TRUE ~ "Other" 
  ))
...
poi <- poi[, .(Severity, `Start Time`, City, State, `Temperature(F)`,
                         Amenity, Bump, Crossing, `Give Way`, Junction, `No Exit`,
...
poi_true <- poi %>% 
  select(Amenity:`Turning Loop`) %>% 
  pivot_longer(Amenity:`Turning Loop`, names_to = "PoI") %>% 
  filter(value == 1) %>%
  group_by(PoI) %>% 
  summarise(t = n()) %>% 
  arrange(desc(t))
...
poi_sum <- poi[, PoI_Sum := rowSums(.SD), .SDcols = c("Amenity", "Bump","Crossing", "Give Way", "Junction", "No Exit", "Railway", "Roundabout", "Station",
                                                        "Stop", "Traffic Calming",
                                                        "Traffic Signal", "Turning Loop")]
...
```{r}
#| label: pré-processando as variaveis do grupo 'Duração do acidente'


duracao_acidente <- duracao_acidente %>%
...
acidentes_ano_barplot <- acidentes_US_date_sample[Civil_Twilight == "Day" | Civil_Twilight == "Night",
                                            .(.N,
...
acidentes_ano_barplot_CT <- acidentes_US_date_sample[Civil_Twilight == "Day" | Civil_Twilight == "Night", .N, by = .(Year, Civil_Twilight)][
...
acidentes_US_date_sample[, .N, by = .(Weekday, Time_Range)] %>% 
...
acidentes_data_matriz_AWKTR <- acidentes_US_date_sample[Civil_Twilight == "Day" | Civil_Twilight == "Night", .N, by = .(Weekday, Time_Range)] %>%
...
```{r}
condicoes_count <- condicoes_climaticas %>%
...
```{r}
climatic_data <- condicoes_climaticas %>%
  select(`Temperature(F)`, `Humidity(%)`, `Pressure(in)`, `Wind Speed(mph)`,`Visibility(mi)`, `Wind Chill(F)`) %>%
  na.omit()
...
```{r}
ggplot(condicoes_climaticas, aes(x = `Precipitation(in)`)) +
  geom_histogram(binwidth = 0.1, fill = "blue", color = "black") +
  labs(title = "Distribuição de Precipitação", x = "Precipitação (in.)", y = "Contagem")


ggplot(condicoes_climaticas, aes(x = `Visibility(mi)`)) +
...
```{r}
accident_chuva <- condicoes_climaticas %>%
  group_by(Choveu) %>%
...
```{r}
  ggplot(condicoes_climaticas, aes(x = `Precipitation(in)`, y = Severity)) +
...
```{r}
barplot <- acidentes_US_sample %>% select(Amenity:`Turning Loop`) %>%
...
```{r}
ggplot(duracao_acidente, aes(x = as.factor(Severity), y = Duration_minutes)) +
  ...
  labs(
    ...
    scale_y_continuous(limits = quantile(duracao_acidente$Duration_minutes, c(0.0, 0.9)))+
  theme_minimal()
```
...
acidentes_mes_barplot <- acidentes_US_date_sample[Civil_Twilight == "Day" | Civil_Twilight == "Night",
                                            .(.N,
                                              Distinct_date = uniqueN(Date),
                                              Media_diaria = round(.N/uniqueN(Date),0)),
                                            by = Month][
...
data_timeseries <- acidentes_US_date_sample[Civil_Twilight == "Day" | Civil_Twilight == "Night", .(.N), by = .(Date)] %>% 
...
acidentes_data_timeseries_CT <- acidentes_US_date_sample[Civil_Twilight == "Day" | Civil_Twilight == "Night", .(.N), by = .(Date, Civil_Twilight)] %>% 
...
acidentes_data_timeseries_6_18 <- acidentes_US_date_sample[Day_Period == "06:00:00 -> 18:00:00" | Day_Period == "18:00:00 -> 06:00:00", .(.N), by = .(Date, Day_Period)] %>% 
...
severity_data_points <- acidentes_US_date_sample[(Civil_Twilight == "Day" | Civil_Twilight == "Night") & Year > 2016,
...
acidentes_data_matriz_SWKTR <- acidentes_US_date_sample[Civil_Twilight == "Day" | Civil_Twilight == "Night", .(.N, Severity = mean(Severity)), by = .(Weekday, Time_Range)] %>%
  setorder(Weekday, -Time_Range) %>% # order data set increasingly by weekday and time range
  as.data.frame() %>% # Transforming data.table into data.frame
  ggplot() + 
...
```{r}
   poi_Severity <- acidentes_US_sample[,c(3,30:42)]
    ...
    poi_sum <- poi_Severity[, Total := rowSums(.SD),
                            .SDcols = c("Amenity", "Bump", "Crossing", "Give Way", "Junction",
